...
class DataTable
{
	 ...
	 *  @return array          Server-side processing response array
	 */
	static function complex($request, $conn, $table, $primaryKey, $columns, $whereResult = null, $whereAll = null, array $otherField = null)
	{
		$bindings = array();
		$db = self::db($conn);
		...
		$data = self::sql_exec(
			...
			$bindings,
			"SELECT `" . implode("`, `", self::pluck($columns, 'db')) . "`" .
				self::formOtherField($otherField) .
				"FROM `$table`
			 $where
			 $order
			 $limit"
	...
	static function formOtherField(array $otherField = null)
	{
		// if (!is_null($otherField)&&count($otherField)>0) {
		//     return  ",`".implode("`,`", $otherField)."` ";
		// } else {
		//     return '';
		// }
		return '';
